...
class Aggregator(nn.Module):
    ...
    def forward(self, entity_emb, user_emb,  
                edge_index, edge_type, interact_mat, weight):
        ...
        # + entity_emb[head] means residual connection
        neigh_relation_emb = (entity_emb)[tail]  * edge_relation_emb   # + entity_emb[head] # [-1, channel]
...
class GraphConv(nn.Module):
    ...
    def forward(self, user_emb, entity_emb, entity_2nd_emb, user_2nd_emb, edge_index, edge_type,
        ...
        entity_layer_embed = [entity_emb]
        user_layer_embed = [user_emb]
...
class Recommender(nn.Module):
    ...
    def generate(self, traning_epoch=-1):
        user_emb = self.all_embed[:self.n_users, :]
        item_emb = self.all_embed[self.n_users:, :]
        return self.gcn(user_emb,
                        ...
                        self.edge_type,
                        self.interact_mat,
                        mess_dropout=False, node_dropout=False)[:3]
